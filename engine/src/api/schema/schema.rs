						...
						TypeDefinition::Object(d) => {
							if d.name == info.name {
								for field in &d.fields {
									fields.push(build_field(info, registry, field.name, field.field_type.clone(), true));
								}
							}
						}
						_ => todo!()
		...
		T: Text<'t>
{
	match type_ref
	{
		...
		{
			build_field(info, registry, field_name, *nested_type, nullable)
		},
